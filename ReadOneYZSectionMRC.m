function slice=ReadOneYZSectionMRC(filename,section_no_X)
...
%       section_no_X: Specify the Section Number along the YZ plane (X=constant) to extract from the 3D EM
%                   Scan.
% Output:
...
% For generating error if the given section number along the YZ plane is more than the total
% number of available sections (i.e. maximum number of columns)
if section_no_X > NX
    error([num2str(section_no_X), ' is beyond the total number of sections(', num2str(NX),').']);
end

slice=zeros(NZ,NY); % Initializing a zero matrix for storing the data of given section.
k=0;
section_no_X=double(section_no_X);
size=double(size);
k=double(k);
NX=double(NX);

% Converting the 1D data into 2D data.
for i=1:NZ
    for j=1:NY
        fseek(ID,(1024)+double(((section_no_X-1)*size)+(k*NX*size)),'bof');
        raw_data=fread(ID,1,datatype,endian); 
        slice(i,j)=raw_data;
        k=k+1;
    end
end
