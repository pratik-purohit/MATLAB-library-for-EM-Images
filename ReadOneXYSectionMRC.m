function slice=ReadOneXYSectionMRC(filename,section_no)
% This function reads specified one section in XY plane from an MRC file and stores it in a 2D array for further usage. 
%
% Usage:
...
%       section_no: Specify the Section Number to extract from the 3D EM
%                   Scan.
% Output:
%       slice: A 2D array containing pixel values corresponding to specified section_no.
%
%
...
% For generating error if the given section number is more than the total
% number of available sections
if section_no > NZ
    error([num2str(section_no), ' is beyond the total number of sections(', num2str(NZ),').']);
end

section_no=double(section_no);
NX=double(NX);
NY=double(NY);
size=double(size);

fseek(ID,double((1024)+((section_no-1)*(NX*NY*size))),'bof'); % To set the address from the data about given section-number starts
raw_data=fread(ID,NX*NY,datatype,endian); % Reading data about the given section-number in 1D array.
fclose(ID); % Closing the file after getting the data.

slice=zeros(NY,NX); % Initializing a zero matrix for storing the data of given section.

% Converting the 1D data into 2D data. Row by Row.
for i=1:NY
    row_start=(1+((i-1)*NX));
    row_end=(i*NX);
    slice(i,:)=raw_data(row_start:row_end);
end
